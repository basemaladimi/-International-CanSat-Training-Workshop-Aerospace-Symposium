...
SERIAL_PORT = "COM4"
BAUD_RATE = 115200
CAMERA_URL = "http://192.168.4.1/capture"  # Adjust to correct ESP32 camera endpoint
...
class SensorApp:
    def __init__(self, root):
        ...
        self.camera_label = tk.Label(cam_frame, text="Camera Loading...",
                                     width=self.camera_width, height=self.camera_height, bg="white")
        ...
        # Centering trick: use grid and .columnconfigure/.rowconfigure
    ...
    def camera_thread(self):
        while self.camera_running:
            try:
                img_bytes = urllib.request.urlopen(CAMERA_URL, timeout=0.5).read()
                img = Image.open(io.BytesIO(img_bytes))
                img = img.resize((self.camera_width, self.camera_height))
                tk_img = ImageTk.PhotoImage(img)
                self.camera_image = tk_img
            except Exception as e:
                self.camera_image = None
            time.sleep(0.03)
    ...
    def update_camera_label(self):
        if self.camera_image:
            self.camera_label.config(image=self.camera_image, text="", bg="white")
            self.camera_label.image = self.camera_image
        else:
            self.camera_label.config(text="Camera not available", bg="white")
        if self.camera_running:
            self.root.after(30, self.update_camera_label)
