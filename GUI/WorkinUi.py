...
class SensorApp:
    def __init__(self, root):
        ...
        # --- REPLACE BUTTON WITH EMBEDDED BROWSER USING CEF ---
        self.browser_frame = tk.Frame(cam_frame, width=self.camera_width, height=self.camera_height, bg="gray")
        self.browser_frame.pack_propagate(False)
        self.browser_frame.pack()
        self.init_cef_browser(self.browser_frame, CAMERA_WEB_URL)
    ...
    def init_cef_browser(self, tk_frame, url):
        # --- CEF must be initialized once for the whole process ---
        if not hasattr(self, "_cef_initialized"):
        ...
        # Embed browser in the given tk_frame
        window_info = cef.WindowInfo()
        window_info.SetAsChild(tk_frame.winfo_id(), [0, 0, self.camera_width, self.camera_height])
        self.browser = cef.CreateBrowserSync(window_info, url=url)
        # Resize CEF browser with tk_frame
        ...
        tk_frame.bind("<Configure>", on_configure)
    ...
    def on_close(self):
        ...
        if self.ser:
            self.ser.close()
        self.root.destroy()
        if hasattr(self, "_cef_initialized"):
            cef.Shutdown()

if __name__ == "__main__":
    # Init CEF before Tk mainloop
    root = tk.Tk()
    app = SensorApp(root)
    root.mainloop()
